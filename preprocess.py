"""One-shot preprocessor: xlsx export -> data/tickets.parquet."""
...
def main() -> int:
    ...
    if "description_text" in df.columns:
        df["description_text"] = (
            df["description_text"].astype("string").str.slice(0, 500)
        )
    ...
    if "sentiment_score" in df.columns:
        ...
        print(f"  sentiment_score range: [{lo}, {hi}]  mean={s.mean():.1f}")
        # Observed range is 0..100 (Freshdesk scaled). Mean ~26.
        # Bottom quartile -> Negative, middle -> Neutral, top -> Positive.
    ...
    if "quarter" in df.columns:
        print("\nrows per quarter:")
        for q, n in df["quarter"].value_counts().sort_index().items():
            print(f"  {q}: {n:>6,}")
    for col in [
        "custom_fields.cf_service",
        "custom_fields.cf_products",
        "custom_fields.cf_root_cause",
        "custom_fields.cf_issue_source",
        "custom_fields.cf_area_of_impact",
        "custom_fields.cf_platform",
        "status_label",
        "priority_label",
        "source_label",
        "sentiment_bucket",
